...
房间的开放/受保护性质由**首个加入者**决定：

| 首个加入者 | 房间性质 | 后续加入条件 |
|---|---|---|
| 不带 `pwd` | **开放房间** | 任何人凭房间号直接进（与旧版一致） |
| 带 `pwd` | **受保护房间** | 必须房间号 + 密码匹配 |

规则：
- **无密码 = 完全开放**：客户端 `join` 不带 `pwd` 字段时，报文与旧版逐字节一致。
- 客户端仅在**用户填了密码**时才发 `pwd`（明文，限长）；服务器存
  `sha256("room:pwd")`，不回传、不落明文。
- 校验（服务器，锁内原子完成，防并发创建 TOCTOU）：
  - 房间开放、客户端却带 `pwd` → `ROOM_OPEN`
  - 房间受保护、客户端未带 `pwd` → `NEED_PASSWORD`
  - 密码不匹配 → `BAD_PASSWORD`
- 房间**空了即彻底删除**（成员表 + 密码一起清除）。
- 网络切换重建（`rebind`）时客户端**重发密码**。
