...
- **新增**：**靶子检查**——打洞入口（精准打洞 / 多轮重试）先检查是否有可达候选
  （`pub_tcp` 非空，或 `lan` 非空）。都没有 = 打也白打，主动请求映射、仍无则放弃本轮，
  等对端登记映射后重新触发（`member_update` / `punch_go`）。
- **新增**：**双连接去重**——同时打开可能形成两条独立 TCP 连接（各方向一条），
  两端各用一条 → 半开。按 ID 规则约定只保留一条主连接：
  ID 大者的出站连接 = 主；ID 小者的入站连接 = 主。
- **优化**：**映射观测连接持续重试**——由固定 3 次改为后台线程持续重试
  （间隔递增，上限 3s），网络抖动时自愈，不再整轮失败。
- **优化**：入站连接识别增加**按公网 IP 回退匹配**——NAT 给"映射 socket"与
  "打洞 socket"分配不同公网端口时，仍能正确认领入站连接。
- **服务器（`server/`）**：
  - **新增**：映射登记后**主动广播 `member_update`**——其他成员立即拿到刚登记的
    `pub_tcp` 可打洞，消除"等对端再发 punch_req"的时序竞态。
  - **新增**：映射连接断开后 **`pub_tcp` 延迟清理**（`TCP_MAPPING_GRACE_MS`，180s）——
    客户端主动关闭映射 socket 释放本地端口时，NAT 映射仍存活，靶子仍有效。
  - **新增**：**`did` 兜底复用 id**——客户端崩溃重启丢失 myId 时，同设备复用旧 id，
    对端看到"同一成员回归"，不产生"离开 + 加入"抖动。
- **协议**：新增信令消息类型 `member_update`（服务器 → 客户端）。
  旧客户端忽略未知类型，向后兼容。
- 覆盖 Python 端、Kotlin 端与服务器。
...
- **新增**：端到端示例 `examples/echo.py`——加入房间 → 打洞 → 收发文本消息 → 回显，
  最贴近真实用法的完整可运行示例（局域网发现示例仍见 `examples/basic_lan.py`）。
- **新增**：协议编解码精确布局（`DPMP_PROTOCOL.md` §7）——UDP-RTP 包头逐字节
  偏移表、各控制报文 payload 结构、v1↔v2 版本协商流程、test_vectors.json 解读。
- **新增**：类型注解 + `py.typed`（PEP 561）——全部公开 API 加类型标注，
  IDE（PyCharm / VSCode）可获得参数与返回值补全。
- **文档**：更新信令服务器说明——它随本项目一起开源（见仓库 `server/` 目录），
  不再是"独立开源"。
- 本次不改变任何协议与传输行为，库功能与 0.1.1 一致；仅新增示例、文档与类型信息。
